...
$(function() {
	...
	/**
	 * Mobile header icon animation & displaying content
	 */
	$('.mobile-header__toggler').click(function() {
		$('.mobile-header__toggler').toggleClass('hidden');
		$('.mobile-header__logo').toggleClass('hidden');
		$('.mobile-header__center-content').toggleClass('hidden');
		$('.mobile-header__content').toggleClass('open');
	})

	/**
	 * Language dropdown
	 */
	$(".lang-dropdown__toggler").click(function(e) { 
		e.preventDefault();
		e.stopPropagation();
		$(this).parents('.lang-dropdown').find('.lang-dropdown__menu').toggleClass("hidden");
	});
	$(document).click(function() {
		$('.lang-dropdown__menu').addClass("hidden");
	})
	...
	$(window).scroll(function() {
		if (window.pageYOffset > headerOffset && window.innerWidth > 767)
	...
	/**
	 * Visible-unvisible search input
	 */
	$('.shop-nav__link-search').on('click', function(e) {
		$(".header__nav-main").toggleClass("hidden");
		$(".header__nav-shop").toggleClass("hidden");
		$(".header__search").toggleClass("hidden");
	});
	$('.shop-nav-search__reset').on('click', function(e) {
		$(".header__nav-main").toggleClass("hidden");
		$(".header__nav-shop").toggleClass("hidden");
		$(".header__search").toggleClass("hidden");
	});
	 ...
	 * Product tabs
	 */
	$('.shop-tabs').on('click', '.shop-tabs__item:not(.active)', function(e) {
		$(this).addClass('active').siblings().removeClass('active');
		$('.shop-tabs__list').find('.shop-tabs__content').removeClass('active').eq($(this).index()).addClass('active');
	});
	...
	$('.splash-slider').slick({
		...
		fade: true,
		cssEase: 'linear',
		prevArrow: $(".splash-slider__btn-prev"),
		nextArrow: $(".splash-slider__btn-next"),
	});
	...
	/**
	 * Colorizing "for her"/"for him" when hover on title
	 */
	$('.her-him__item-link').hover(function() {
		$(this).parents('.her-him__item').find('.her-him__item-image-link').addClass('active');
	}, function() {
		$(this).parents('.her-him__item').find('.her-him__item-image-link').removeClass('active');
	});
	...
	$('.filters__group-title').on('click', function() {
		...
		$(this).hasClass('list-shown') ?
			$(this).next().slideDown().addClass('list-shown') :
			$(this).next().slideUp();
	});
	 ...
	 * Filtering Products
	 */
	$('.filters__btn-apply').on('click', () => {
		/**
		 * As we have two filter blocks(mobile & desktop) - we need to define which checked checboxes we need to grab.
		 */
		var filterHolderClass = 'filters_desktop';
		if( $(window).outerWidth() < 992 ) {
			filterHolderClass = 'filters_mobile';
		}

		var $filters = {
			genders: get_filter(filterHolderClass, 'filters__gender'),
			types: get_filter(filterHolderClass, 'filters__type'),
			brands: get_filter(filterHolderClass, 'filters__brand'),
		}
		...
		function get_filter(filtersHolderClass, class_name) {
			var filter = [];
			$('.' + filtersHolderClass + ' .' + class_name + ':checked').each(function(){
	...
	$('.filters_mobile .filters__title').on('click', function() {
		$(this).next().hasClass('shown') ? 
			$(this).next().slideUp().removeClass('shown') :
			$(this).next().slideDown().addClass('shown');

			$(this).next().find('.filters__group-title').each(function() {
				if( !$(this).next().hasClass('list-shown') )
					$(this).next().slideDown().addClass('list-shown');
			});
	});
	 ...
	 */
	$('a.sort-by__link').on('click', function() {
		var $order = $(this).find('input.sort-by__val').val();

		/**
		 * For development puprposes
		 */
		alert($order);
	...
	/**
	 * Pager
	 * See the docs: http://flaviusmatis.github.io/simplePagination.js
	 */
    $('#pager').pagination({
		items: 25,
		displayedPages: 3,
	...
	$( "#contact-form .form-control" ).each(function() {
		$(this)
			.mouseenter(function() {
				$(this).parents('.contact-form').addClass('hover');
			})
			.mouseleave(function() {
				$(this).parents('.contact-form').removeClass('hover');
			})
			.focusin(function() {
				$(this).parents('.contact-form').addClass('focus');
			})
			.focusout(function() {
				$(this).parents('.contact-form').removeClass('focus');
			})
			
	});

	$('.single-product__minus').on('click', (event) => {
		const valueInput = $(event.currentTarget)
			.parents('.single-product__count')
			.find('.single-product__count-value')[0];

		if(valueInput ) {
			const nextValue = valueInput.value - 1;
	...
	$('.single-product__plus').on('click', (event) => {
		const valueInput = $(event.currentTarget)
			.parents('.single-product__count')
			.find('.single-product__count-value')[0];

		// We also can set maximum value we can order
		if(valueInput && valueInput.value < 9999) {
			valueInput.value = +valueInput.value + 1;
		}
	});

	// Accept only numbers in product count number
	$('.single-product__count-value').on('keyup', (event) => { 
		if($(event.target)[0]) {
			$(event.target)[0].value = $(event.target)[0].value.replace(/[^0-9]/g, '');
		}
	});

	$('.cart-table__remove-item').on('click', (event) => {
		if($(event.currentTarget)) {
			$(event.currentTarget).parents('.cart-table__row').remove();
		}
	})

	$('#cash-on-delivery, #bank-transfer, #payment-on-spot')
		.attr('disabled', 'true')
		.parent().addClass('disabled');

	$('#ceska-posta, #slovenska-posta').on('click', () => {
		$('#payment-on-spot')
			.attr('disabled', true)
			.prop('checked', false)
			.parent().addClass('disabled');
	...
	$('.single-product-front-images').slick({
		slidesToShow: 1,
		slidesToScroll: 1,
		arrows: false,
		fade: true,
	...
	$('.single-product-thumbs').slick({
		slidesToShow: 3,
		slidesToScroll: 1,
		asNavFor: '.single-product-front-images',
		vertical: true,
		dots: false,
		centerMode: true,
		focusOnSelect: true,
...
var autocompleteData = [{
	title: 'Alexandre J Oscent Rouge EDP 100ml',
	price: '4000 Ck',
	img: 'img/product-1.png',
	url: '//google.com'
}];

var autoCompleteDesktop = autoCompleteConstructor('#search__input_desktop', '.shop-nav-search__results_desktop'),
	autoCompleteMobile = autoCompleteConstructor('#search__input_mobile', '.shop-nav-search__results_mobile');

function autoCompleteConstructor(searchInput, destination) {
	return new autoComplete({
		...
		placeHolder: "Поиск среди 100+ товаров...",
		...
		resultsList: {
			...
			container: function(source) {
				resultsListClass = "row";
				return resultsListClass;
			},
		...
		resultItem: {
			content: function(data, source) {
				source.className = 'col-sm-6 col-md-4 shop-nav-search__results-item';
				source.innerHTML = '<a href="' + data.value.url + '" class="shop-nav-search__results-image-link"> \
										<img src="' + data.value.img + '" alt="' + data.value.title +'" class="img-responsive"> \
									</a> \
									<a href="' + data.value.url + '" class="second-back first-hover-back shop-nav-search__results-info"> \
										<span class="first-color shop-nav-search__results-title">'
											+ data.match + 
										'</span> \
										<span class="first-color shop-nav-search__results-price">'
											+ data.value.price +
										'</span> \
									</a>'
			},
